...
Traccar.app.getAttributePreference('volumeUnit'))},volumeConverter:function(value){return Ext.getStore('VolumeUnits').convertValue(value,Traccar.app.getAttributePreference('volumeUnit'))},durationFormatter:function(value){return Math.floor(value/36E5)+' '+Strings.sharedHourAbbreviation+' '+Math.floor(value%36E5/6E4)+' '+Strings.sharedMinuteAbbreviation},deviceIdFormatter:function(value){return Ext.getStore('Devices').getById(value).get('name')},groupIdFormatter:function(value){if(0!==value){var group=
Ext.getStore('AllGroups');0===group.getTotalCount()&&(group=Ext.getStore('Groups'));return(group=group.getById(value))?group.get('name'):value}return null},geofenceIdFormatter:function(value){if(0!==value){var store=Ext.getStore('AllGeofences');0===store.getTotalCount()&&(store=Ext.getStore('Geofences'));return(value=store.getById(value))?value.get('name'):''}return null},driverUniqueIdFormatter:function(value){if(0!==value){var driver=Ext.getStore('AllDrivers');0===driver.getTotalCount()&&(driver=
Ext.getStore('Drivers'));return(driver=driver.findRecord('uniqueId',value,0,!1,!0,!0))?value+' ('+driver.get('name')+')':value}return null},lastUpdateFormatter:function(value){if(value){value=Math.floor((new Date-value)/1E3);0>value&&(value=0);var interval=Math.floor(value/86400);if(1<interval)return interval+' '+Strings.sharedDays;interval=Math.floor(value/3600);return 1<interval?interval+' '+Strings.sharedHours:Math.floor(value/60)+' '+Strings.sharedMinutes}return null},commandTypeFormatter:function(value){var name=
...
'key';break;case 'speedUnit':combobox.store='SpeedUnits';combobox.displayField='name';combobox.valueField='key';break;case 'volumeUnit':combobox.store='VolumeUnits';combobox.displayField='fullName';combobox.valueField='key';break;case 'timezone':combobox.store='AllTimezones',combobox.displayField='key'}else combobox.xtype='textfield'}valueField.getXType()!==combobox.xtype?(this.getView().down('form').insert(this.getView().down('form').items.indexOf(valueField),combobox),this.getView().down('form').remove(valueField)):
'customNumberField'===combobox.xtype?(valueField.setConfig(combobox),valueField.validate()):'combobox'===combobox.xtype&&(valueField.setConfig(combobox),valueField.setValue())}}});
...
!1)&&!Traccar.app.getUser().get('admin');var readonly=Traccar.app.getPreference('readonly',!1)&&!Traccar.app.getUser().get('admin');this.lookupReference('toolbarAddButton').setDisabled(readonly||deviceReadonly);this.lookupReference('toolbarDeviceMenu').setHidden(readonly||deviceReadonly);setInterval(function(){self.getView().getView().refresh()},Traccar.Style.refreshPeriod)},onCommandClick:function(){var deviceId=this.getView().getSelectionModel().getSelection()[0].get('id');var dialog=Ext.create('Traccar.view.dialog.SendCommand');
dialog.deviceId=deviceId;var typesStore=dialog.lookupReference('commandsComboBox').getStore();typesStore.getProxy().setExtraParam('deviceId',deviceId);Traccar.app.getPreference('limitCommands',!1)||typesStore.add({id:0,description:Strings.sharedNew});typesStore.load({addRecords:!0});typesStore=dialog.lookupReference('commandType').getStore();typesStore.getProxy().setExtraParam('deviceId',deviceId);typesStore.load();dialog.show()},updateButtons:function(selected){var deviceReadonly=Traccar.app.getPreference('deviceReadonly',
!1)&&!Traccar.app.getUser().get('admin');var readonly=Traccar.app.getPreference('readonly',!1)&&!Traccar.app.getUser().get('admin');var empty=0===selected.length;this.lookupReference('toolbarEditButton').setDisabled(empty||readonly||deviceReadonly);this.lookupReference('toolbarRemoveButton').setDisabled(empty||readonly||deviceReadonly);deviceReadonly=this.lookupReference('toolbarDeviceMenu');deviceReadonly.device=empty?null:selected[0];deviceReadonly.setDisabled(empty);this.lookupReference('deviceCommandButton').setDisabled(empty||
...
void 0!==this.period&&dialog.lookupReference('periodField').setValue(this.period);dialog.show()},updateButtons:function(){var reportType=this.lookupReference('reportTypeField').getValue();var disabled=this.deviceId&&0!==this.deviceId.length||this.groupId&&0!==this.groupId.length;var time=this.fromDate&&this.fromTime&&this.toDate&&this.toTime;disabled=!reportType||!disabled||!time;this.lookupReference('showButton').setDisabled(disabled);this.lookupReference('exportButton').setDisabled('chart'===reportType||
disabled)},onReportClick:function(button){var reportType;this.getGrid().getSelectionModel().deselectAll();if((reportType=this.lookupReference('reportTypeField').getValue())&&(this.deviceId||this.groupId)){var from=new Date(this.fromDate.getFullYear(),this.fromDate.getMonth(),this.fromDate.getDate(),this.fromTime.getHours(),this.fromTime.getMinutes(),this.fromTime.getSeconds(),this.fromTime.getMilliseconds());var to=new Date(this.toDate.getFullYear(),this.toDate.getMonth(),this.toDate.getDate(),this.toTime.getHours(),
this.toTime.getMinutes(),this.toTime.getSeconds(),this.toTime.getMilliseconds());'showButton'===button.reference?('chart'===reportType?(button=this.getChart().getStore(),this.getChart().setSeries([])):button=this.getGrid().getStore(),button.showMarkers=this.showMarkers,button.load({params:{deviceId:this.deviceId,groupId:this.groupId,type:this.eventType,from:from.toISOString(),to:to.toISOString()}})):'exportButton'===button.reference&&(button=this.getGrid().getStore().getProxy().url,this.downloadFile(button,
{deviceId:this.deviceId,groupId:this.groupId,type:this.eventType,from:Ext.Date.format(from,'c'),to:Ext.Date.format(to,'c')}))}},onClearClick:function(){var reportType=this.lookupReference('reportTypeField').getValue();this.clearReport(reportType)},clearReport:function(reportType){this.getGrid().getStore().removeAll();'trips'!==reportType&&'events'!==reportType&&'stops'!==reportType||Ext.getStore('ReportRoute').removeAll();'chart'===reportType&&this.getChart().getStore().removeAll()},onSelectionChange:function(selection,
selected){0<selected.length&&(selection=selected[0],this.fireEvent('selectreport',selection,!0),selection instanceof Traccar.model.ReportTrip&&this.selectTrip(selection),(selection instanceof Traccar.model.Event||selection instanceof Traccar.model.ReportStop)&&this.selectPositionRelated(selection))},selectDevice:function(device){device&&this.getGrid().getSelectionModel().deselectAll()},selectReport:function(object){var reportType=this.lookupReference('reportTypeField').getValue();if(object instanceof
Traccar.model.Position)if('route'===reportType)this.getGrid().getSelectionModel().select([object],!1,!0),this.getGrid().getView().focusRow(object);else if('events'===reportType||'stops'===reportType)object=this.getGrid().getStore().findRecord('positionId',object.get('id'),0,!1,!0,!0),this.getGrid().getSelectionModel().select([object],!1,!0),this.getGrid().getView().focusRow(object)},deselectFeature:function(){'trips'!==this.lookupReference('reportTypeField').getValue()&&this.getGrid().getSelectionModel().deselectAll()},
selectTrip:function(trip){var from=new Date(trip.get('startTime'));var to=new Date(trip.get('endTime'));Ext.getStore('ReportRoute').removeAll();Ext.getStore('ReportRoute').showMarkers=this.showMarkers;Ext.getStore('ReportRoute').load({params:{deviceId:trip.get('deviceId'),from:from.toISOString(),to:to.toISOString()}})},selectPositionRelated:function(report){report.get('positionId')&&(report=Ext.getStore('ReportRoute').getById(report.get('positionId')))&&this.fireEvent('selectreport',report,!0)},loadRelatedPositions:function(store,
...
binary:!0,success:function(response){var disposition=response.getResponseHeader('Content-Disposition');disposition=disposition.slice(disposition.indexOf('\x3d')+1,disposition.length);var type=response.getResponseHeader('Content-Type');response=new Blob([response.responseBytes],{type:type});if('undefined'!==typeof window.navigator.msSaveBlob)window.navigator.msSaveBlob(response,disposition);else{var url=window.URL||window.webkitURL;var downloadUrl=url.createObjectURL(response);disposition&&Ext.dom.Helper.append(Ext.getBody(),
{tag:'a',href:downloadUrl,download:disposition}).click();setTimeout(function(){url.revokeObjectURL(downloadUrl)},100)}}})},onTypeChange:function(combobox,newValue,oldValue){null!==oldValue&&this.clearReport(oldValue);'route'===newValue?(this.getGrid().reconfigure('ReportRoute',this.routeColumns),this.getView().getLayout().setActiveItem('grid')):'events'===newValue?(this.getGrid().reconfigure('ReportEvents',this.eventsColumns),this.getView().getLayout().setActiveItem('grid')):'summary'===newValue?
(this.getGrid().reconfigure('ReportSummary',this.summaryColumns),this.getView().getLayout().setActiveItem('grid')):'trips'===newValue?(this.getGrid().reconfigure('ReportTrips',this.tripsColumns),this.getView().getLayout().setActiveItem('grid')):'stops'===newValue?(this.getGrid().reconfigure('ReportStops',this.stopsColumns),this.getView().getLayout().setActiveItem('grid')):'chart'===newValue&&this.getView().getLayout().setActiveItem('chart');this.updateButtons()},onGeocode:function(positionId){var position=
Ext.getStore('ReportRoute').getById(positionId);position&&!position.get('address')&&Ext.Ajax.request({scope:this,method:'GET',url:'api/server/geocode',params:{latitude:position.get('latitude'),longitude:position.get('longitude')},success:function(response){position.set('address',response.responseText);position.commit();this.fireEvent('selectReport',position)},failure:function(response){Traccar.app.showError(response)}})},routeColumns:[{text:Strings.reportDeviceName,dataIndex:'deviceId',renderer:Traccar.AttributeFormatter.getFormatter('deviceId')},
{text:Strings.positionValid,dataIndex:'valid',renderer:Traccar.AttributeFormatter.getFormatter('valid')},{text:Strings.positionFixTime,dataIndex:'fixTime',xtype:'datecolumn',renderer:Traccar.AttributeFormatter.getFormatter('fixTime')},{text:Strings.positionLatitude,dataIndex:'latitude',renderer:Traccar.AttributeFormatter.getFormatter('latitude')},{text:Strings.positionLongitude,dataIndex:'longitude',renderer:Traccar.AttributeFormatter.getFormatter('longitude')},{text:Strings.positionAltitude,dataIndex:'altitude',
renderer:Traccar.AttributeFormatter.getFormatter('altitude')},{text:Strings.positionSpeed,dataIndex:'speed',renderer:Traccar.AttributeFormatter.getFormatter('speed')},{text:Strings.positionAddress,dataIndex:'address',renderer:function(value,metaData,record){return value?Traccar.AttributeFormatter.getFormatter('address')(value):'\x3ca href\x3d"#" onclick\x3d"Ext.fireEvent(\'routegeocode\', '+record.getId()+')" \x3e'+Strings.stateShowAddress+'\x3c/a\x3e'}}],eventsColumns:[{text:Strings.positionFixTime,
dataIndex:'serverTime',xtype:'datecolumn',renderer:Traccar.AttributeFormatter.getFormatter('serverTime')},{text:Strings.reportDeviceName,dataIndex:'deviceId',renderer:Traccar.AttributeFormatter.getFormatter('deviceId')},{text:Strings.sharedType,dataIndex:'type',renderer:function(value){return Traccar.app.getEventString(value)}},{text:Strings.sharedGeofence,dataIndex:'geofenceId',renderer:Traccar.AttributeFormatter.getFormatter('geofenceId')}],summaryColumns:[{text:Strings.reportDeviceName,dataIndex:'deviceId',
renderer:Traccar.AttributeFormatter.getFormatter('deviceId')},{text:Strings.sharedDistance,dataIndex:'distance',renderer:Traccar.AttributeFormatter.getFormatter('distance')},{text:Strings.reportAverageSpeed,dataIndex:'averageSpeed',renderer:Traccar.AttributeFormatter.getFormatter('speed')},{text:Strings.reportMaximumSpeed,dataIndex:'maxSpeed',renderer:Traccar.AttributeFormatter.getFormatter('speed')},{text:Strings.reportEngineHours,dataIndex:'engineHours',renderer:Traccar.AttributeFormatter.getFormatter('duration')},
...
Ext.define('Traccar.view.map.BaseMap',{extend:Ext.panel.Panel,xtype:'baseMapView',layout:'fit',getMap:function(){return this.map},getMapView:function(){return this.mapView},initMap:function(){var server=Traccar.app.getServer();var type=Traccar.app.getPreference('map',null);var bingKey=server.get('bingKey');switch(type){case 'custom':server=new ol.layer.Tile({source:new ol.source.XYZ({url:server.get('mapUrl'),attributions:''})});break;case 'bingRoad':server=new ol.layer.Tile({source:new ol.source.BingMaps({key:bingKey,
...
var feature=this.accuracyCircles[position.get('deviceId')];if(position.get('accuracy')){var projection=radius.getProjection();var center=ol.proj.fromLonLat([position.get('longitude'),position.get('latitude')]);projection=ol.proj.getPointResolution(projection,radius.getResolution(),center);radius=position.get('accuracy')/ol.proj.METERS_PER_UNIT.m*radius.getResolution()/projection;feature?(feature.getGeometry().setCenter(center),feature.getGeometry().setRadius(radius)):(feature=new ol.Feature(new ol.geom.Circle(center,
...
style.getImage().zoom,style.getImage().angle,category);category=style.getText();category.setOffsetY(-color.getSize()[1]/2-Traccar.Style.mapTextOffset);style.setText(category);style.setImage(color)},selectMarker:function(marker,center){this.selectedMarker&&(this.selectedMarker.get('event')?(this.getView().getMarkersSource().removeFeature(this.selectedMarker),marker&&marker.get('event')||this.fireEvent('deselectevent')):!Ext.getStore('ReportRoute').showMarkers&&this.selectedMarker.get('record')instanceof
Traccar.model.Position?(this.getView().getMarkersSource().removeFeature(this.selectedMarker),delete this.reportMarkers[this.selectedMarker.get('record').get('id')]):(this.resizeMarker(this.selectedMarker.getStyle(),!1),this.selectedMarker.getStyle().setZIndex(0),this.selectedMarker.changed()));marker&&(this.resizeMarker(marker.getStyle(),!0),marker.getStyle().setZIndex(1),marker.changed(),center&&(this.getView().getMapView().setCenter(marker.getGeometry().getCoordinates()),0!==this.selectZoom&&this.selectZoom>
this.getView().getMapView().getZoom()&&this.getView().getMapView().setZoom(this.selectZoom)));this.selectedMarker=marker},selectDevice:function(device,center){this.selectMarker(this.latestMarkers[device.get('id')],center)},selectReport:function(position,center){position instanceof Traccar.model.Position&&(Ext.getStore('ReportRoute').showMarkers||(this.reportMarkers[position.get('id')]=this.addReportMarker(position)),this.selectMarker(this.reportMarkers[position.get('id')],center))},selectEvent:function(position){this.fireEvent('deselectfeature');
position?(position=this.addReportMarker(position),position.set('event',!0),this.selectMarker(position,!0)):this.selectedMarker&&this.selectedMarker.get('event')&&this.selectMarker(null,!1)},selectFeature:function(feature){(feature=feature.get('record'))&&(feature instanceof Traccar.model.Device?this.fireEvent('selectdevice',feature,!1):this.fireEvent('selectreport',feature,!1))},deselectFeature:function(){this.deselectDevice();this.fireEvent('deselectfeature')},deselectDevice:function(){this.selectMarker(null,
!1)},zoomToAllPositions:function(data){var i,maxx,maxy;for(i=0;i<data.length;i++){var point=ol.proj.fromLonLat([data[i].get('longitude'),data[i].get('latitude')]);if(0===i){var minx=maxx=point[0];var miny=maxy=point[1]}else minx=Math.min(point[0],minx),miny=Math.min(point[1],miny),maxx=Math.max(point[0],maxx),maxy=Math.max(point[1],maxy)}minx!==maxx||miny!==maxy?this.getView().getMapView().fit([minx,miny,maxx,maxy]):point&&this.getView().getMapView().fit(new ol.geom.Point(point))},updateVisibleDevices:function(store,
data){Ext.isArray(data)||(data=[data]);for(store=0;store<data.length;store++){var device=data[store];device.get('id')in this.latestMarkers&&this.updateDeviceVisibility(device)}},isDeviceVisible:function(device){return Ext.getStore('VisibleDevices').contains(device)},updateDeviceVisibility:function(device){var deviceId=device.get('id');var marker=this.latestMarkers[deviceId];var accuracy=this.accuracyCircles[deviceId];deviceId=this.liveRoutes[deviceId];this.isDeviceVisible(device)?(marker&&!this.getView().getMarkersSource().getFeatureById(marker.getId())&&
...
var earthSphere=new ol.Sphere(6378137);projection=earthSphere.haversineDistance(result,ol.proj.transform(geometry,projection,'EPSG:4326'));result='CIRCLE ('+(result[1]+' '+result[0]+', ');result+=projection.toFixed(1)+')'}else if(geometry instanceof ol.geom.Polygon){geometry.transform(projection,'EPSG:4326');geometry=geometry.getCoordinates();result='POLYGON((';for(projection=0;projection<geometry[0].length;projection+=1)result+=geometry[0][projection][1]+' '+geometry[0][projection][0]+', ';result=
result.substring(0,result.length-2)+'))'}else if(geometry instanceof ol.geom.LineString){geometry.transform(projection,'EPSG:4326');geometry=geometry.getCoordinates();result='LINESTRING (';for(projection=0;projection<geometry.length;projection+=1)result+=geometry[projection][1]+' '+geometry[projection][0]+', ';result=result.substring(0,result.length-2)+')'}return result}});
...
feature.setStyle(this.getAreaStyle(geofence.get('name'),geofence.get('attributes')?geofence.get('attributes').color:null));this.getView().getGeofencesSource().addFeature(feature);return!0},this)},zoomToAllDevices:function(){this.zoomToAllPositions(Ext.getStore('LatestPositions').getData().items)}});
...
Traccar.app.getUser().get('id')});dialog.down('form').loadRecord(user);dialog.show()},onAddClick:function(){var user=Ext.create('Traccar.model.User');Traccar.app.getUser().get('admin')&&user.set('deviceLimit',-1);Traccar.app.getUser().get('expirationTime')&&user.set('expirationTime',Traccar.app.getUser().get('expirationTime'));var dialog=Ext.create('Traccar.view.dialog.User');dialog.down('form').loadRecord(user);dialog.show()},onDevicesClick:function(){var user=this.getView().getSelectionModel().getSelection()[0];
...
Ext.define('Traccar.view.SettingsMenuController',{extend:Ext.app.ViewController,alias:'controller.settings',init:function(){var admin=Traccar.app.getUser().get('admin');var manager=0!==Traccar.app.getUser().get('userLimit');var readonly=Traccar.app.getPreference('readonly',!1);var deviceReadonly=Traccar.app.getUser().get('deviceReadonly');admin&&(this.lookupReference('settingsServerButton').setHidden(!1),this.lookupReference('settingsStatisticsButton').setHidden(!1));(admin||manager)&&this.lookupReference('settingsUsersButton').setHidden(!1);
...
this.lookupReference('settingsCommandsButton').setHidden(Traccar.app.getPreference('limitCommands',!1));(admin||!deviceReadonly&&!readonly)&&this.lookupReference('settingsComputedAttributesButton').setHidden(Traccar.app.getBooleanAttributePreference('ui.disableComputedAttributes'))},onUserClick:function(){var dialog=Ext.create('Traccar.view.dialog.User',{selfEdit:!0});dialog.down('form').loadRecord(Traccar.app.getUser());dialog.lookupReference('testNotificationButton').setHidden(!1);dialog.show()},
...
Ext.define('Traccar.controller.Root',{extend:Ext.app.Controller,alias:'controller.root',init:function(){var i,chartTypesStore=Ext.getStore('ReportChartTypes');Ext.state.Manager.setProvider(new Ext.state.CookieProvider);var data=Ext.getStore('PositionAttributes').getData().items;for(i=0;i<data.length;i++){var attribute=data[i];Traccar.model.Position.addFields([{name:'attribute.'+attribute.get('key'),attributeKey:attribute.get('key'),calculate:this.calculateAttribute,persist:!1}]);'number'===attribute.get('valueType')&&
chartTypesStore.add({key:'attribute.'+attribute.get('key'),name:attribute.get('name')})}},calculateAttribute:function(data){data=data.attributes[this.attributeKey];return void 0!==data?Traccar.AttributeFormatter.getAttributeConverter(this.attributeKey)(data):data},onLaunch:function(){Ext.Ajax.request({scope:this,url:'api/server',callback:this.onServerReturn})},onServerReturn:function(options,success,response){options={};if(success){Traccar.app.setServer(Ext.decode(response.responseText));if(success=
Ext.Object.fromQueryString(window.location.search).token)options.token=success;Ext.Ajax.request({scope:this,url:'api/session',method:'GET',params:options,callback:this.onSessionReturn})}else Traccar.app.showError(response)},onSessionReturn:function(options,success,response){Ext.get('spinner').setVisible(!1);success?(Traccar.app.setUser(Ext.decode(response.responseText)),this.loadApp()):(this.login=Ext.create('widget.login',{listeners:{scope:this,login:this.onLogin}}),this.login.show())},onLogin:function(){this.login.close();
this.loadApp()},loadApp:function(){var attribution;Ext.getStore('Groups').load();Ext.getStore('Drivers').load();Ext.getStore('Geofences').load();Ext.getStore('Calendars').load();Ext.getStore('ComputedAttributes').load();Ext.getStore('AllCommandTypes').load();Ext.getStore('Commands').load();Ext.getStore('AllNotificationTypes').load({callback:function(records,operation,success){operation=Ext.getStore('ReportEventTypes');success&&(operation.add({type:Traccar.store.ReportEventTypes.allEvents,name:Strings.eventAll}),
operation.loadData(records,!0))}});Ext.getStore('Notifications').load();Ext.getStore('ServerAttributes').loadData(Ext.getStore('CommonDeviceAttributes').getData().items,!0);Ext.getStore('ServerAttributes').loadData(Ext.getStore('CommonUserAttributes').getData().items,!0);Ext.getStore('UserAttributes').loadData(Ext.getStore('CommonUserAttributes').getData().items,!0);Ext.getStore('DeviceAttributes').loadData(Ext.getStore('CommonDeviceAttributes').getData().items,!0);Ext.getStore('GroupAttributes').loadData(Ext.getStore('CommonDeviceAttributes').getData().items,
...
Ext.getCmp('soundButton');return soundButton&&soundButton.pressed},removeUrlParameter:function(param){var params=Ext.Object.fromQueryString(window.location.search);delete params[param];Ext.Object.isEmpty(params)?window.history.pushState(null,null,window.location.pathname):window.history.pushState(null,null,window.location.pathname+'?'+Ext.Object.toQueryString(params))},asyncUpdate:function(first){var self=this;var protocol='https:'===window.location.protocol?'wss:':'ws:';var pathname=window.location.pathname.substring(0,
...
(device=Ext.getStore('VisibleDevices').findRecord('id',array,0,!1,!0,!0))&&this.fireEvent('selectdevice',device,!0),device||this.zoomToAllDevices())},updateEvents:function(array){var i,device;var store=Ext.getStore('Events');for(i=0;i<array.length;i++)if(store.add(array[i]),device=Ext.getStore('Devices').getById(array[i].deviceId))this.soundPressed()&&this.beep(),Traccar.app.showToast(array[i].text,device.get('name'))},zoomToAllDevices:function(){var lat=Traccar.app.getPreference('latitude',0);var lon=
Traccar.app.getPreference('longitude',0);var zoom=Traccar.app.getPreference('zoom',0);0===lat&&0===lon&&0===zoom&&this.fireEvent('zoomtoalldevices')}});
